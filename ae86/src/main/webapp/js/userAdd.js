function noAdd() {
    if (!(accountcheck() && Emailcheck() && phonecheck() && nickNamecheck() && check() && pawEquals() && isCardNo())) {
...
function account(ev) {
    $.toast({
        text: "亲~用户登录账号用于登录，不能为空呦~", // Text that is to be shown in the toast
...
function phonech(ev) {
    $.toast({
        text: "亲~手机号码不能为空或号码不正确呦~", // Text that is to be shown in the toast
        heading: '提示', // Optional heading to be shown on the toast
        icon: 'info', // Type of toast icon
        hideAfter: 1700, // false to make it sticky or number representing the miliseconds as time after which toast needs to be hidden
        loaderBg: '#9EC600'  // Background color of the toast loader
